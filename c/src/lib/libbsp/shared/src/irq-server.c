...
static void bsp_interrupt_server_trigger(void *arg)
{
 ...
 // printf("bsp_interrupt_server_trigger func\n");
...
)
{
  rtems_status_code sc = RTEMS_SUCCESSFUL;
  bsp_interrupt_server_entry *e = NULL;
//printf("rtems_interrupt_server_handler_install\n");
...
//printf("server:%d\n",server );
  ...
  //  printf("server != RTEMS_ID_NONE\n");
    return RTEMS_NOT_IMPLEMENTED;
  }

  if (RTEMS_INTERRUPT_IS_SHARED(options)) {
   // printf("RTEMS_INTERRUPT_IS_SHARED\n");
    return RTEMS_NOT_IMPLEMENTED;
  }
